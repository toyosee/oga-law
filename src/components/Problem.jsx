...
import { motion } from 'framer-motion';
import { AlertTriangle, Megaphone, Search, DollarSign } from 'lucide-react';

const problems = [
  {
    icon: AlertTriangle,
    title: 'No Access to Legal Knowledge',
    description: '70% of Nigerians cannot name 3 fundamental rights guaranteed by the Constitution. When rights are unknown, they cannot be exercised.',
    color: 'text-red-400',
    bg: 'bg-red-400/10',
  },
  {
    icon: Megaphone,
    title: 'Individual Complaints Get Ignored',
    description: 'Without amplification, documented rights violations disappear into the void. Police stations ignore complaints. Government portals are black holes.',
    color: 'text-orange-400',
    bg: 'bg-orange-400/10',
  },
  {
    icon: Search,
    title: 'Search Engines Give Wrong Answers',
    description: 'Search engines return generic, foreign legal information that does not apply to Nigerian law. In a crisis, bad information is worse than no information.',
    color: 'text-yellow-400',
    bg: 'bg-yellow-400/10',
  },
  {
    icon: DollarSign,
    title: 'Extortion Is Normalized',
    description: 'Without knowing your rights, "settlement" and unlawful payments become the default response. Silence becomes survival. Fear becomes culture.',
    color: 'text-ogalaw-gold',
    bg: 'bg-ogalaw-gold/10',
  },
];
    ...
    <section id="about" className="py-24 relative">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-400 max-w-2xl mx-auto text-lg">
            Nigeria has strong laws. But for most citizens, they might as well not exist — 
            and even when violations are documented, individual voices are rarely loud enough to demand accountability.
          </p>
        </motion.div>
